...
class MarketAnalyzer:
    ...
    def clean_data(self) -> None:
        ...
        # For categorical columns, replace nulls with the mode (most frequent value)
        categorical_columns = [
            col for col in self.real_estate_data.columns if self.real_estate_data[col].dtype == pl.Utf8
        ]
        for col in categorical_columns:
            mode = self.real_estate_data[col].mode().item(0)
            self.real_estate_clean_data = self.real_estate_clean_data.with_columns(
                [pl.col(col).fill_null(mode).alias(col)]
            )

    def generate_price_distribution_analysis(self) -> pl.DataFrame:
        """
        Analyze sale price distribution and generate a histogram.

        Returns:
            pl.DataFrame: DataFrame with price statistics
        """
    ...
    def neighborhood_price_comparison(self) -> pl.DataFrame:
        ...
        Returns:
            pl.DataFrame: DataFrame with neighborhood price statistics
        """
        neighborhood_stats = (
            self.real_estate_clean_data
            .group_by("Neighborhood")  # Correct method: group_by()
            .agg(
                [
            ...
            .sort("mean_price", descending=True)  # Sort by mean_price in descending order
        )

        # Plot boxplot using Plotly
        fig = px.box(
            self.real_estate_clean_data.to_pandas(),
            x="Neighborhood",
            y="SalePrice",
...
if __name__ == "__main__":
    ...
    analyzer = MarketAnalyzer(data_path="data/synthetic_train.csv", output_dir="./outputs")
    analyzer.clean_data()  # Clean the data
